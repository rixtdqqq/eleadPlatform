...
package cn.finalteam.mygallery.utils;
...
public class PhotoTools {
     ...
     * @return
     */
    public static List<cn.finalteam.mygallery.model.PhotoFolderInfo> getAllPhotoFolder(Context context, List<cn.finalteam.mygallery.model.PhotoInfo> selectPhotoMap) {
        List<cn.finalteam.mygallery.model.PhotoFolderInfo> allFolderList = new ArrayList<>();
        ...
        final ArrayList<cn.finalteam.mygallery.model.PhotoFolderInfo> allPhotoFolderList = new ArrayList<>();
        HashMap<Integer, cn.finalteam.mygallery.model.PhotoFolderInfo> bucketMap = new HashMap<>();
        Cursor cursor = null;
        //所有图片
        cn.finalteam.mygallery.model.PhotoFolderInfo allPhotoFolderInfo = new cn.finalteam.mygallery.model.PhotoFolderInfo();
        allPhotoFolderInfo.setFolderId(0);
        allPhotoFolderInfo.setFolderName(context.getResources().getString(R.string.all_photo));
        allPhotoFolderInfo.setPhotoList(new ArrayList<cn.finalteam.mygallery.model.PhotoInfo>());
            ...
            if (cursor != null) {
                ...
                while (cursor.moveToNext()) {
                    ...
                    if ( (filterList == null || !filterList.contains(path)) && file.exists() && file.length() > 0 ) {
                        ...
                        //通过bucketId获取文件夹
                        cn.finalteam.mygallery.model.PhotoFolderInfo photoFolderInfo = bucketMap.get(bucketId);

                        if (photoFolderInfo == null) {
                            photoFolderInfo = new cn.finalteam.mygallery.model.PhotoFolderInfo();
                            photoFolderInfo.setPhotoList(new ArrayList<cn.finalteam.mygallery.model.PhotoInfo>());
