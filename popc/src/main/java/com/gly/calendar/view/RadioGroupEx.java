...
/**
 * Created by mjc on 2016/1/20. 重新对RadioGroup进行布局，可以折行 默认水平开始排布
 */
public class RadioGroupEx extends RadioGroup {
	...
	public static Mode mode = Mode.FIXED_COLUMNS;

	public enum Mode {
		AUTOMATIC, FIXED_COLUMNS
	}// 模式1自动换行,模式2手动设置列数
	...
	public RadioGroupEx(Context context, AttributeSet attrs) {
		...
		vSpace = density * 16.5f;
		hSpace = density * 16.5f;
		setPadding(0, (int) vSpace, (int) hSpace, (int) vSpace);
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mode == Mode.FIXED_COLUMNS) {
			deltaX = (int) (widthSize - getPaddingLeft() - getPaddingRight() - hSpace
					* (NUM_COLUMNS))
					/ NUM_COLUMNS;
		}
		...
		for (int i = 0; i < count; i++) {
			...
			if (mode == Mode.AUTOMATIC) {
				deltaX = child.getMeasuredWidth() + params.leftMargin
						+ params.rightMargin;
			} else {
				params.width = deltaX;
			}
			params.setMargins((int) hSpace, 0, 0, (int) vSpace);
			...
			if (lineWidth + deltaX + getPaddingLeft() + getPaddingRight() > widthSize) {// 如果折行,height增加
				...
				maxLineHeight = child.getMeasuredHeight() + params.topMargin
						+ params.bottomMargin;
				Log.v(TAG, "maxHeight:" + totalHeight + "---" + "maxWidth:"
						+ maxWidth);
				...
				lineWidth += deltaX;
				// 不换行，计算行最高
				int deltaY = child.getMeasuredHeight() + params.topMargin
						+ params.bottomMargin;
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (preLeft + params.leftMargin + child.getMeasuredWidth()
					+ params.rightMargin + getPaddingRight() > (r - l)) {
				// 重置
			...
			if (mode == Mode.AUTOMATIC) {
				deltaX = child.getMeasuredWidth();
			}
			// left坐标
			int left = preLeft + params.leftMargin;
			// top坐标
			int top = preTop + params.topMargin;
			int ddRight = left + deltaX;
			int right = getWidth() > ddRight ? ddRight : deltaX;
			int bottom = top + child.getMeasuredHeight();
			// 为子view布局
			child.layout(left, top, right, bottom);
			// 计算布局结束后，preLeft的值
			preLeft += params.leftMargin + deltaX + params.rightMargin;
			child.requestLayout();
