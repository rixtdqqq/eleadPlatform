...
import com.elead.im.controller.EaseUI.EaseEmojiconInfoProvider;
import com.elead.im.controller.EaseUI.EaseSettingsProvider;
import com.elead.im.controller.EaseUI.EaseUserProfileProvider;
import com.elead.im.entry.EMUserForNick;
import com.elead.im.entry.EaseEmojicon;
import com.elead.im.entry.EaseEmojiconGroupEntity;
...
import com.elead.im.entry.InviteMessage.InviteMesageStatus;
import com.elead.im.entry.RobotUser;
import com.elead.im.model.EaseAtMessageHelper;
import com.elead.im.model.EaseNotifier;
import com.elead.im.model.EaseNotifier.EaseNotificationInfoProvider;
import com.elead.im.parse.UserProfileManager;
import com.elead.im.receiver.CallReceiver;
import com.elead.im.util.EaseCommonUtils;
import com.elead.im.util.PreferenceManager;
import com.elead.im.util.RedPacketUtil;
import com.elead.im.widget.DemoModel;
...
public class DemoHelper {
    ...
    public void init(Context context) {
        ...
        if (EaseUI.getInstance().init(context, options)) {
            ...
            initDbDao();
            if (SharedPreferencesUtil.getInt(context, Util.LOGIN_COUNT) > 0) {
                initConnectPeople();
            }
        }
    }

    private void initConnectPeople() {
        ...
        VHttpUrlConnectionUtil.postRequest(url, parameters, new HttpUrlConnectionUtil.onConnectionFinishLinstener() {
            ...
            public void onSuccess(String url, String result) {
                Log.d("initConnectPeople", result);
                try {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        EMUserForNick epUser = JSON.parseObject(array.get(i).toString(), EMUserForNick.class);
    ...
    public Map<String, EaseUser> getContactMap() {
        if (isLoggedIn() && contactList == null) {
            ...
            for (Map.Entry<String, EaseUser> easeUserMap : contactList.entrySet()) {
                {
                    if (easeUserMap.getValue().getUsername().equals(getCurrentUsernName())) {
                        contactList.remove(easeUserMap.getValue().getUsername());
    ...
    public List<EaseUser> getArrayContactList() {
        List<EaseUser> contactList = new ArrayList<>();
        Map<String, EaseUser> contactsMap = getContactMap();
        if (contactsMap == null) {
            return contactList;
        }
