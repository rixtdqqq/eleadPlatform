...
/**
 * @desc 这是一个类似支付宝声波支付的波纹效果布局,该布局中默认添加了不可见的圆形的视图,启动动画时会启动缩放、颜色渐变动画使得产生波纹效果.
 * 这些动画都是无限循环的,并且每个View的动画之间都有时间间隔，这些时间间隔就会导致视图有大有小，从而产生波纹的效果.
 * @auth Created by Administrator on 2016/12/2 0002.
 */
public class RippleLayout extends RelativeLayout {
    ...
    private static final int DEFAULT_RIPPLE_COLOR = Color.rgb(0x7d, 0x66, 0xbd);
    private static final int DEFAULT_STROKE_WIDTH = 0;
    private static int DEFAULT_RADIUS = 33;
    ...
    private void initTypedArray(Context context, AttributeSet attrs) {
        DEFAULT_RADIUS = DimenUtils.dip2px(context,12);
        mRippleRadius = DEFAULT_RADIUS;
        final TypedArray typedArray = context.obtainStyledAttributes(attrs,
                R.styleable.RippleLayout);
        //
        mRippleColor = typedArray.getColor(R.styleable.RippleLayout_color_bg,
